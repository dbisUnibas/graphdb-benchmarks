...
public class Utils {
     ...
     * @param config for individual databases.
     */
    ...
    public static void writeResults( File output, Map<GraphDatabaseType, List<Double>> times, String benchmarkTitle ) {
        ...
        try ( BufferedWriter out = new BufferedWriter( new FileWriter( output ) ) ) {
            ...
            for ( GraphDatabaseType type : times.keySet() ) {
                out.write( String.format( "%s,%f,%d,%f\n", type.getShortname(), means.get( type ), times.get( type ).size(), standardDeviations.get( type ) ) );
            }
        } catch ( IOException e ) {
            throw new BenchmarkingException( String.format( "Exception thrown when writing output to %s: %s", output, e.getMessage() ) );
        }
    }
