...
public abstract class BenchmarkBase implements Benchmark {
    ...
    protected final void createDatabases() {
        for ( GraphDatabaseType type : bench.getSelectedDatabases() ) {
            logger.info( String.format( "creating %s database from %s dataset", type.getShortname(), bench.getDataset().getName() ) );
            File dbpath = Utils.generateStorageDirectory( type, bench.getDbStorageDirectory() );
            if ( dbpath.exists() ) {
                throw new IllegalStateException( String.format( "Database from a previous run exist: %s; clean up and try again.", dbpath.getAbsolutePath() ) );
            }
